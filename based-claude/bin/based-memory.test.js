...
test("new --write creates a governed draft with provenance and readers", () => {
  const root = tempRoot();
  const result = runScript("based-memory.js", [
    "new",
    "--title", "Test Card",
    "--scope", "repo",
    "--summary", "A summary.",
  ...
  const draftPath = path.join(root, ".based/memory/cards/drafts/test-card.md");
...
test("promote requires --approved", () => {
  const root = tempRoot();
  runScript("based-memory.js", [
    "new", "--title", "Needs Approval", "--scope", "repo", "--summary", "Claim.",
    "--evidence", "e1", "--source", "s1", "--write", "--root", root,
  ]);
  const result = runScript("based-memory.js", ["promote", "needs-approval", "--root", root]);
...
test("promote supersedes the draft: it moves to active and is removed from drafts", () => {
  ...
  const draftPath = path.join(root, ".based/memory/cards/drafts/promo-card.md");
  assert.ok(fs.existsSync(draftPath));
  fs.writeFileSync(draftPath, fillPlaceholders(fs.readFileSync(draftPath, "utf8")), "utf8");

  const result = runScript("based-memory.js", ["promote", "promo-card", "--approved", "--root", root]);
  assert.equal(result.status, 0, result.stdout + result.stderr);

  const activePath = path.join(root, ".based/memory/cards/active/promo-card.md");
...
test("retire supersedes the active card: it moves to retired and stops matching retrieve", () => {
  ...
  const draftPath = path.join(root, ".based/memory/cards/drafts/retire-card.md");
  fs.writeFileSync(draftPath, fillPlaceholders(fs.readFileSync(draftPath, "utf8")), "utf8");
  runScript("based-memory.js", ["promote", "retire-card", "--approved", "--root", root]);
  const activePath = path.join(root, ".based/memory/cards/active/retire-card.md");
  assert.ok(fs.existsSync(activePath), "precondition: card must be active before retiring");

  const before = runScript("based-memory.js", ["retrieve", "--query", "widgets", "--root", root]);
  assert.match(before.stdout, /retire-card/, "precondition: retrieve should find the active card");

  const retireResult = runScript("based-memory.js", ["retire", "retire-card", "--approved", "--root", root]);
  assert.equal(retireResult.status, 0, retireResult.stdout + retireResult.stderr);

  const retiredPath = path.join(root, ".based/memory/cards/retired/retire-card.md");
  ...
  const after = runScript("based-memory.js", ["retrieve", "--query", "widgets", "--root", root]);
...
test("audit --strict fails a card with empty required sections", () => {
  ...
  const result = runScript("based-memory.js", ["audit", "--strict", "--root", root]);
  assert.notEqual(result.status, 0);
...
test("audit --recovery flags an active card that its own applies_when cues cannot find", () => {
  const root = tempRoot();
  const activeDir = path.join(root, ".based/memory/cards/active");
...
`;
  ...
  const result = runScript("based-memory.js", ["audit", "--recovery", "--root", root]);

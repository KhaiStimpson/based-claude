...
test("based-plan prints to stdout and writes nothing without --write", () => {
  const root = tempRoot();
  const result = runScript("based-plan.js", ["--title", "no write plan", "--root", root]);
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /# no write plan/i);
  assert.ok(!fs.existsSync(path.join(root, ".based")), "no files should be written without --write");
});

test("based-plan --write creates the full progressive bundle", () => {
  const root = tempRoot();
  const result = runScript("based-plan.js", [
  ...
  assert.equal(result.status, 0, result.stderr);
  const dir = path.join(root, ".based/plans/auth-refactor");

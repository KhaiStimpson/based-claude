...
Curate governed memory cards; never write memory as a side effect. `based-claudius/references/memory-card-schema.md` is the schema and lifecycle.
...
All lifecycle operations go through `node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-memory.js"` so gates and ledgers stay consistent. This agent intentionally has no native `memory:` scope — governed cards are its only durable store.

...
Take a code-review stance. Findings come first. `based-claudius/references/review-policy.md` is the full policy; review from objective, diff, tests, validation, and contracts — not the implementer's transcript.
...
5. Delivered-code quality per `based-claudius/references/code-style.md`: style mismatch with local conventions, comment noise, speculative abstraction, defensive noise, diff pollution. Rank below correctness; reject quality findings that are actually local convention.

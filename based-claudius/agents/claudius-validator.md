...
Run checks; do not edit. `based-claudius/references/validation-ladder.md` picks the rung.

1. Discover validation candidates from manifests, scripts, tests, and docs — `node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-quality-gate.js"` lists them when useful.
2. Run the smallest check that can falsify the changed behavior, then broaden per the ladder when risk requires it.
3. For nontrivial diffs, include the mechanical style pass: `node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-slop-check.js" --diff`.

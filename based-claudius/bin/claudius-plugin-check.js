...
function harnessScorecard() {
  const rows = [
    ...
    {
      component: "memory",
      evidence: ["references/memory-card-schema.md", "bin/claudius-memory.js"],
      ok: hasText("references/memory-card-schema.md", /provenance/i) && hasText("bin/claudius-memory.js", /supersession-reviewed/),
    },
    {
      component: "verification",
      evidence: ["references/validation-ladder.md", "bin/claudius-quality-gate.js"],
      ok: hasText("references/validation-ladder.md", /Syntax and schema/) && hasFile("bin/claudius-quality-gate.js"),
    },
    {
      component: "code quality",
      evidence: ["references/code-style.md", "bin/claudius-slop-check.js"],
      ok: hasText("references/code-style.md", /The Codebase Wins/) && hasFile("bin/claudius-slop-check.js"),
    },
    {
      component: "observability",
      evidence: ["references/trace-schema.md", "bin/claudius-trace.js"],
      ok: hasText("references/trace-schema.md", /risk_class/) && hasText("bin/claudius-trace.js", /risk-class/),
    },
    {
...
if (fs.existsSync(path.join(root, ".claudius-smoke"))) {
  errors.push("Remove .claudius-smoke from the plugin source/package surface before validation.");
}
...
for (const script of [
  "_lib.js",
  "claudius-doctor.js",
  "claudius-quality-gate.js",
  "claudius-slop-check.js",
  "claudius-plan.js",
  "claudius-handoff.js",
  "claudius-memory.js",
  "claudius-trace.js",
  "claudius-improve.js",
  "claudius-plugin-check.js",
]) {
  requireFile(path.join(binDir, script), `bin script ${script}`);
}

const memoryScript = readText(path.join(binDir, "claudius-memory.js"));
for (const marker of ["missing provenance evidence", "missing allowed_readers", "duplicate active claim", "supersession-reviewed"]) {
  if (!memoryScript.includes(marker)) errors.push(`claudius-memory.js missing governance marker: ${marker}`);
}

const improveScript = readText(path.join(binDir, "claudius-improve.js"));
for (const marker of ["reviews.jsonl", "requiresIndependentReview", "--allow-shell", "reviewRequired"]) {
  if (!improveScript.includes(marker)) errors.push(`claudius-improve.js missing governance marker: ${marker}`);
}
for (const script of ["claudius-quality-gate.js", "claudius-improve.js"]) {
...
const ruleCopyPairs = [
  {
    ...
    fileB: "agents/claudius-developer.md",
    phrases: [
  ...
  },
  {
    ...
    fileB: "agents/claudius-safety.md",
...
for (const token of ["claudius-doctor", "claudius-quality-gate", "claudius-slop-check", "claudius-plan", "claudius-memory", "claudius-trace", "claudius-improve", "/based-claudius:work", "/based-claudius:check"]) {
  if (!readme.includes(token)) warnings.push(`README does not mention ${token}`);
}

console.log("# Based Claudius Plugin Check");
console.log("");

...
`claudius-slop-check` reports narration comments, placeholder debris, AI-tell phrases,
...
`bin/claudius-slop-check.test.js` covers:
...
Run: `npm test` (or `node --test bin/claudius-slop-check.test.js`) from the plugin root.

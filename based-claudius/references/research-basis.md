# Based Claudius Research Basis

This reference distills the local MAS Research Wiki as inspected on 2026-07-05. Treat it as design guidance for the plugin, not as live proof about a target repository.

## Core Thesis

Production coding agents are not just code-writing prompts. They are tool-using systems with context budgets, action boundaries, observable state, validation harnesses, memory policies, repair loops, and review checkpoints. A good developer experience keeps the machinery light until the work justifies it — and the delivered code indistinguishable from what a careful maintainer would write.
...
8. Self-improvement must be reversible.
   ...
   - Promotion requires before/after evidence, versioning, rollback, retirement criteria, and independent review for evaluator or safety changes. Ungated trace-to-skill pipelines are the documented ecosystem failure mode, not a shortcut.
...
11. Instruction structure beats instruction volume.
    - Instruction-file studies show added prose helps roughly as often as it hurts; engineered structure (one canonical source per policy, progressive loading, compact routing surfaces) is what moves outcomes.
    - Delivered-code quality is a contract, not a vibe: `code-style.md` states it once and validation enforces it, instead of scattering style adjectives across prompts.
...
- `wiki/plans/based-claude-plugin-improvement-plan.md`
- `wiki/sources/summary-toward-instructions-as-code-understanding-the-impact-of-instruction-files-on-agentic-pull-.md`

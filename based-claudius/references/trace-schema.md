...
Based Claudius uses compact action-state traces so memory and self-improvement can inspect validated work without replaying full transcripts.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-trace.js" append --objective "..." --event validation --summary "..." --validation pass --commands "npm run check"
node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-trace.js" append --objective "..." --event workflow --summary "mode=direct owner=claudius-developer delegates=none" --decisions "broad but single context"
node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-trace.js" append --objective "..." --event review --summary "broad discovery complete" --review-stage discovery --effort high
node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-trace.js" summarize
node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-trace.js" list --limit 10
```

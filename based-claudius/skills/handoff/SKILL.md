...
3. Print the handoff in chat; write it to `.claudius/handoffs/<slug>.md` only on request or when continuation crosses sessions: `node "${CLAUDE_PLUGIN_ROOT}/bin/claudius-handoff.js" --write`.
4. Update `.claudius/state/current.md` when it exists, so the resume point is one file.

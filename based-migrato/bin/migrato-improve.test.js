...
test("propose --write creates a proposal with rollback and retirement fields", () => {
  const root = tempRoot();
  const result = runScript("migrato-improve.js", [
    "propose",
    "--title", "Fix example",
  ...
  const proposalPath = path.join(root, ".migrato/improvements/proposals/fix-example.md");
...
test("promote requires --approved", () => {
  const root = tempRoot();
  runScript("migrato-improve.js", [
    "propose", "--title", "Needs Approval", "--problem", "p", "--change", "c", "--evidence", "e", "--write", "--root", root,
  ]);
  const result = runScript("migrato-improve.js", ["promote", "needs-approval", "--root", root]);
...
test("promote refuses without a passing evaluation", () => {
  ...
  const result = runScript("migrato-improve.js", ["promote", "no-eval", "--approved", "--root", root]);
  assert.notEqual(result.status, 0);
});
...
test("a passing evaluation plus promote accepts a proposal with no risky target", () => {
  ...
  const evalResult = runScript("migrato-improve.js", [
    "evaluate", "docs-tweak", "--command", PASS_CMD, "--write", "--root", root,
  ]);
  assert.equal(evalResult.status, 0, evalResult.stdout + evalResult.stderr);

  const promoteResult = runScript("migrato-improve.js", ["promote", "docs-tweak", "--approved", "--root", root]);
  assert.equal(promoteResult.status, 0, promoteResult.stdout + promoteResult.stderr);
  assert.ok(fs.existsSync(path.join(root, ".migrato/improvements/accepted/docs-tweak.md")));
});
...
test("a failing evaluation blocks promotion", () => {
  ...
  const promoteResult = runScript("migrato-improve.js", ["promote", "broken-change", "--approved", "--root", root]);
...
test("evaluate --command blocks shell metacharacters without --allow-shell, and runs them with it", () => {
  ...
  const blocked = runScript("migrato-improve.js", [
  ...
  const allowed = runScript("migrato-improve.js", [
...
test("a proposal touching a script or hook requires independent review before promotion", () => {
  ...
  runScript("migrato-improve.js", ["evaluate", "hook-change", "--command", PASS_CMD, "--write", "--root", root]);

  const blocked = runScript("migrato-improve.js", ["promote", "hook-change", "--approved", "--root", root]);
  assert.notEqual(blocked.status, 0, "should require independent review for a hook-touching change");

  const reviewed = runScript("migrato-improve.js", [

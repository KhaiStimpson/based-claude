...
const memoryRoot = path.resolve(root, args.dir || ".migrato/memory");
...
const traceFile = path.resolve(root, args.trace || ".migrato/traces/actions.jsonl");
...
Usage:
  migrato-memory new --title "..." --scope repo --summary "..." --evidence "..." [--write]
  migrato-memory suggest [--write] [--limit 20] [--from-successes]
  migrato-memory promote <slug-or-file> --approved [--supersession-reviewed] [--force]
  migrato-memory retrieve --query "..." [--scope repo,user] [--limit 5]
  migrato-memory retire <slug-or-file> --approved
  migrato-memory audit [--strict] [--recovery]
  migrato-memory list [--status active|draft|retired]
  migrato-memory show <slug-or-file>

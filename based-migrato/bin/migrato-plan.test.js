...
test("migrato-plan prints to stdout and writes nothing without --write", () => {
  const root = tempRoot();
  const result = runScript("migrato-plan.js", ["--title", "no write plan", "--root", root]);
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /# no write plan/i);
  assert.ok(!fs.existsSync(path.join(root, ".migrato")), "no files should be written without --write");
});

test("migrato-plan --write creates the full progressive bundle", () => {
  const root = tempRoot();
  const result = runScript("migrato-plan.js", [
  ...
  assert.equal(result.status, 0, result.stderr);
  const dir = path.join(root, ".migrato/plans/auth-refactor");

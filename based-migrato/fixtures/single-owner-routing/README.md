...
| "explore how the auth module handles sessions across this large unfamiliar repo" | delegate to `migrato-scout` |
| "the login test is failing, fix it" | repair mode with a diagnostic ledger (direct or `migrato-repairer`) |
| "review my diff before I open a PR" | delegate to `migrato-reviewer` |
| "plan a migration to a new auth provider" | `discover -> plan`, delegate to `migrato-planner` |
| "add a debug log line" | `discover -> act -> verify`, no delegation |
| "rotate these API credentials" | safety gate before `act` (`migrato-safety`) |

...
Use this schema for durable cards under `.migrato/memory/cards/` or another explicitly approved project memory store.

Default layout:

```text
.migrato/memory/cards/drafts/
.migrato/memory/cards/active/
.migrato/memory/cards/retired/
.migrato/memory/promotions.jsonl
.migrato/memory/retirements.jsonl
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" suggest --write
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" promote card-slug --approved --supersession-reviewed
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" retrieve --query "test command" --scope repo,user
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" retire card-slug --approved
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" audit --strict
node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-memory.js" audit --recovery
```

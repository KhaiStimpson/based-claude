...
- `opus`: reserved for hard reasoning and planning — `migrato-planner` runs on it because it is only invoked when work is broad, ambiguous, or architectural. `opusplan` (opus during plan mode, sonnet for execution) is a session-level model setting, not a subagent frontmatter value.
- `haiku`: cheap read-only exploration — `migrato-scout` runs on it; keep haiku-facing prompts short, literal, and free of judgment calls.

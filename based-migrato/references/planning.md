...
## Migration Is The Default Task Shape

Based Migrato's primary job is migrating legacy pages to a new implementation. When the request is "migrate page X" (or otherwise reads as a legacy-to-new port), plan it as a migration per `migration.md`: enumerate the legacy page's full feature set, reconcile against the `.migrato/migration/component-map.md` registry, and emit **horizontal, individually shippable and testable slices**, each carrying feature-parity assertions tracked in `.migrato/migration/parity.md`. Read `migration.md` before producing a migration plan — it owns the slice bar, the parity ledger lifecycle, and the file schemas. The rest of this contract still applies for non-migration planning.
...
A complete plan states objective, scope, proposed changes, validation, risks, stop conditions, assumptions, and next actions. It identifies whether work should stay direct or use a subagent role, and why. For a migration plan it also states the legacy feature enumeration, the component-map reconciliation (including unmapped gaps), the ordered slice sequence with each slice's isolation mechanism and independent validation, and the per-slice parity assertions (`migration.md`). It states:

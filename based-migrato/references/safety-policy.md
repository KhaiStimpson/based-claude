...
For live tool adapters, prefer structural controls over prompt-only promises: read-only by construction where possible, writes disabled by default, explicit confirmation for writes, proof-after-write, quarantine for repeated failures, and sanitized external logs or tool output. `tool-adapter-safety.md` carries the full checklist.
...
- Prefer an established scanner over ad hoc reading for anything beyond a small, trusted skill — static analysis plus semantic review catches classes of hidden payload manual reading misses.

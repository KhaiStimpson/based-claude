...
2. Run the smallest falsifying check first; broaden per the ladder as risk requires. `node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-quality-gate.js"` lists local candidates.
3. For nontrivial diffs, run the mechanical style pass: `node "${CLAUDE_PLUGIN_ROOT}/bin/migrato-slop-check.js" --diff` (or explicit paths).

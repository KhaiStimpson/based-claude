...
const memoryRoot = path.resolve(root, args.dir || ".based/memory");
...
const traceFile = path.resolve(root, args.trace || ".based/traces/actions.jsonl");
...
Usage:
  based-memory new --title "..." --scope repo --summary "..." --evidence "..." [--write]
  based-memory suggest [--write] [--limit 20] [--from-successes]
  based-memory promote <slug-or-file> --approved [--supersession-reviewed] [--force]
  based-memory retrieve --query "..." [--scope repo,user] [--limit 5]
  based-memory retire <slug-or-file> --approved
  based-memory audit [--strict]
  based-memory list [--status active|draft|retired]
  based-memory show <slug-or-file>

Writes are explicit. New and suggest print drafts unless --write is provided. Promote and retire require --approved.`);
}
...
function retrieve() {
  ...
  for (const file of cardsByStatus("active")) {
    const text = readText(file);
    const meta = parseFrontmatter(text);
    if (meta.scope && !scopes.has(meta.scope)) continue;
